...
- `_ds_bundle.js` — the whole-DS bundle at the project root; loads every component to `window.QuireInkReader`. First line is a `/* @ds-bundle: … */` metadata header.
...
Components are then available at `window.QuireInkReader.*`. Mount into a dedicated child node (e.g. `<div id="ds-root">`), not the host page's own React root, so the two trees don't collide:

```jsx
const { Component } = window.QuireInkReader;
ReactDOM.createRoot(document.getElementById('ds-root')).render(<Component />);
```

## Tokens

48 CSS custom properties from quireink. Names are
preserved verbatim from upstream. They are declared inside `_ds_bundle.css` (this DS ships one compiled stylesheet rather than separate token files).

- **spacing** (2): `--rail-gap`, `--rail-pad`
- **typography** (3): `--font-sans`, `--font-reading`, `--font-mono`
- **other** (43): `--gallery-ratio`, `--gallery-w`, `--gallery-cap`, …
