import type { Dict } from './types'

const de = {
  emptyPosts: 'Noch keine Beiträge.',
  frontFeatured: 'Empfohlen',
  frontPopular: 'Meistgelesen',
  frontLatest: 'Neueste',
  frontAllPosts: 'Alle Beiträge',
  emptyCategory: 'Noch keine Beiträge in dieser Kategorie.',
  emptyTag: 'Noch keine Beiträge mit diesem Schlagwort.',
  categoryLabel: 'Kategorie',
  tagLabel: 'Schlagwort',
  skipToContent: 'Zum Inhalt springen',
  menu: 'Menü',
  // Short tokens for the header controls, shown INSTEAD of the icons when the IDE chrome
  // is on. Brackets come from the sheet, never from here.
  shortSearch: '/suche',
  shortTheme: 'dunkel',
  shortGrid: 'raster',
  shortMail: '@mail',
  shortMenu: 'menü',
  shortPalette: 'farbe',
  gridView: 'Rasteransicht',
  listView: 'Listenansicht',
  palette: 'Farbschema',
  paletteNames: { mono: 'Mono', sepia: 'Sepia', forest: 'Wald', ocean: 'Ozean', scifi: 'Sci-Fi', amber: 'Bernstein' },
  theme: 'Design',
  themeLight: 'Hell',
  themeDark: 'Dunkel',
  themeSystem: 'System',
  themeTime: 'Nach Uhrzeit',
  readingSuffix: 'Min. Lesezeit',
  wordsSuffix: 'Wörter',
  bylinePrefix: 'von',
  updatedPrefix: 'Aktualisiert',
  authorAbout: 'Über den Autor',
  bookMode: 'Buchmodus',
  bookModeClose: 'Schließen',
  bookModePrev: 'Vorherige Seite',
  bookModeNext: 'Nächste Seite',
  bookModeSmaller: 'Kleinere Schrift',
  bookModeLarger: 'Größere Schrift',
  search: 'Suche',
  searchHint: 'Tippen, um Beiträge zu suchen.',
  searchEmpty: 'Keine passenden Beiträge gefunden.',
  searchResults: '{n} Ergebnisse für "{q}"',
  pagerNewer: 'Neuer',
  pagerOlder: 'Älter',
  pagerLabel: 'Seitennavigation',
  pagerPage: 'Seite {n}',
  metaTerm: 'Alle Beiträge auf {site}, die unter {name} stehen.',
  metaSeries: '{name}: eine Serie auf {site}, in der Reihenfolge, in der sie gelesen werden will.',
  tocIndex: 'Inhalt',
  categoriesTitle: 'Kategorien',
  mostViewedTitle: 'Meistgelesen',
  featuredTitle: 'Empfohlen',
  tagsTitle: 'Schlagwörter',
  relatedTitle: 'Ähnliche Beiträge',
  readNext: 'Weiterlesen',
  readNextSeries: 'Nächster Teil der Serie',
  resumePrompt: 'Dort weiterlesen, wo du aufgehört hast?',
  seriesLabel: 'Serie',
  seriesPartPrefix: 'Teil',
  seriesTitle: 'Serien',
  emptySeries: 'Noch keine Beiträge in dieser Serie.',
  // /archive: the year index, its jump row and its empty state. `archiveTitle` is also
  // the sidebar block's heading, so the page and the way in carry one name.
  archiveTitle: 'Archiv',
  archiveMeta: 'Alle Beiträge auf {site}, nach Jahren.',
  archiveYears: 'Jahre',
  archiveEmpty: 'Noch nichts im Archiv.',
  // /notes: the owner's notebook (ADR 0044) — its heading, its description, its empty state,
  // and the word before a clip's source.
  notesTitle: 'Notizen',
  notesMeta: 'Notizen und Ausschnitte, aufbewahrt auf {site}.',
  notesEmpty: 'Noch keine Notizen.',
  noteSourcePrefix: 'Aus',
  // /notes/clip: the notebook's receiving door and its bookmarklet; the last three ride
  // the reader's pen on another site.
  clipHeading: 'Im Notizbuch behalten',
  clipTitleLabel: 'Titel',
  clipNoteLabel: 'Deine Notiz',
  clipPrivate: 'Privat',
  clipPublic: 'Öffentlich',
  clipSave: 'Behalten',
  clipSavedHeading: 'Behalten.',
  clipSavedView: 'Notiz öffnen',
  clipAgain: 'Noch eine behalten',
  clipToolHeading: 'Von überall behalten',
  clipToolHint: 'Zieh das in deine Lesezeichenleiste. Markiere auf einer beliebigen Seite eine Passage und klick darauf: sie landet hier, mitsamt ihrer Herkunft.',
  clipToolLabel: 'In {site} behalten',
  readerPenSend: 'An mein Notizbuch senden',
  readerPenNotebookAsk: 'Wo ist dein Quire Ink? Seine Adresse:',
  readerPenNotebookGo: 'Senden',
  previewNotice: 'Vorschau: Diese Seite ist nicht öffentlich und wird nicht indexiert.',
  nlInvalid: 'Das sieht nicht nach einer E-Mail-Adresse aus.',
  nlNoMail: 'Angemeldet. E-Mail ist nicht konfiguriert, daher wurde keine Bestätigung gesendet.',
  copyCode: 'Kopieren',
  copiedCode: 'Kopiert',
  nlHeading: 'Newsletter abonnieren',
  nlPlaceholder: 'deine@email.de',
  nlButton: 'Abonnieren',
  nlSuccess: 'Fast geschafft — bitte bestätige in deinem Posteingang.',
  nlError: 'Etwas ist schiefgelaufen. Bitte versuche es erneut.',
  nlConfirmSubject: 'Abo bestätigen',
  nlConfirmIntro: 'Bestätige dein Abo von {site}.',
  nlConfirmButton: 'Abo bestätigen',
  nlConfirmIgnore: 'Falls du das nicht angefordert hast, ignoriere diese E-Mail.',
  nlThanksTitle: 'Du bist dabei!',
  nlThanksBody: 'Danke fürs Bestätigen — du hörst von uns, sobald es etwas Neues gibt.',
  nlLinkInvalid: 'Dieser Link ist ungültig oder abgelaufen.',
  nlUnsubTitle: 'Abgemeldet',
  nlUnsubBody: 'Du erhältst keine E-Mails mehr. Du kannst dich jederzeit wieder anmelden.',
  nlUnsubFooter: 'Abmelden',
  nlFooterWhy: 'Du erhältst diese E-Mail, weil du {site} abonniert hast.',
  nlUnsubConfirm: 'Bestätige, dass du keine E-Mails mehr erhalten möchtest.',
  nlUnsubConfirmBtn: 'Abmeldung bestätigen',
  mailTestSubject: 'SMTP-Test',
  mailTestBody: 'Dies ist eine Test-E-Mail von deinem Blog. Wenn du sie lesen kannst, funktioniert SMTP.',
  mailTestSamplePost: 'Beispielbeitrag',
  bcastRead: 'Beitrag lesen',
  bcastDigestSubject: '{n} neue Beiträge',
  replySubject: 'Neue Antwort auf deinen Kommentar',
  replyIntro: '{name} hat auf deinen Kommentar zu "{title}" geantwortet.',
  replyRead: 'Antwort ansehen',
  notFoundTitle: 'Seite nicht gefunden',
  notFoundText: 'Die gesuchte Seite existiert nicht oder wurde verschoben.',
  errorTitle: 'Etwas ist schiefgelaufen',
  errorText: 'Ein unerwarteter Fehler ist aufgetreten. Bitte versuche es erneut.',
  backHome: 'Zur Startseite',
  backToTop: 'Nach oben',
  quoteCopy: 'Zitat kopieren',
  quoteCopied: 'Kopiert',
  readerPenHighlight: 'Markieren',
  readerPenUnderline: 'Unterstreichen',
  readerPenRing: 'Einkreisen',
  readerPenNote: 'Notiz',
  readerPenDelete: 'Markierung entfernen',
  readerPenNoteHint: 'Deine Notiz, in diesem Browser gespeichert',
  readerPenKeptHere: 'Nur in diesem Browser gespeichert.',
  readerPenKeep: 'Auf jedem Gerät behalten',
  readerPenKeepGoogle: 'Mit Google anmelden',
  readerPenKeepCode: 'Code holen',
  readerPenKeepHave: 'Schon einen Code? Hier einfügen:',
  readerPenKeepUse: 'Verwenden',
  readerPenKept: 'Auf jedem Gerät gespeichert.',
  readerPenKeepHint: 'Schreib dir diesen Code auf. Er ist der einzige Schlüssel zu deinen Markierungen und gilt auf jedem Gerät.',
  readerPenForgetHere: 'Auf diesem Gerät vergessen',
  readerPenForgetAll: 'Überall vergessen',
  readerPenKeepBad: 'Dieser Code ist hier nicht bekannt.',
  readerPenShowCode: 'Code anzeigen',
  lightboxPrev: 'Vorheriges Bild',
  lightboxNext: 'Nächstes Bild',
  lightboxClose: 'Schließen',
  commentsHeading: 'Kommentare',
  commentsEmpty: 'Noch keine Kommentare. Sei der Erste.',
  commentName: 'Name',
  commentEmail: 'E-Mail',
  commentEmailNote: 'Nicht veröffentlicht',
  commentWebsite: 'Website (optional)',
  commentBody: 'Dein Kommentar',
  commentSubmit: 'Kommentar senden',
  commentReply: 'Antworten',
  commentDeleted: '[Kommentar gelöscht]',
  commentError: 'Kommentar konnte nicht gesendet werden. Bitte erneut versuchen.',
  commentChecking: 'Browser wird geprüft, einen Moment.',
  commentSignInGoogle: 'Mit Google anmelden',
  commentAs: 'Kommentieren als',
  commentSignOut: 'Abmelden',
  commentSignInError: 'Anmeldung fehlgeschlagen. Bitte erneut versuchen oder mit Name und E-Mail kommentieren.',
} satisfies Dict

export default de
