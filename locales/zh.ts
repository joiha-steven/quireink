import type { Dict } from './types'

const zh = {
  emptyPosts: '还没有文章。',
  frontFeatured: '精选',
  frontPopular: '最多阅读',
  frontLatest: '最新',
  frontAllPosts: '全部文章',
  emptyCategory: '该分类下还没有文章。',
  emptyTag: '该标签下还没有文章。',
  categoryLabel: '分类',
  tagLabel: '标签',
  skipToContent: '跳到内容',
  menu: '菜单',
  // Short tokens for the header controls, shown INSTEAD of the icons when the IDE chrome
  // is on. Brackets come from the sheet, never from here.
  shortSearch: '/搜索',
  shortTheme: '暗色',
  shortGrid: '网格',
  shortMail: '@邮件',
  shortMenu: '菜单',
  shortPalette: '配色',
  gridView: '网格视图',
  listView: '列表视图',
  palette: '配色',
  paletteNames: { mono: '单色', sepia: '棕褐', forest: '森林', ocean: '海洋', scifi: '科幻', amber: '琥珀' },
  theme: '主题',
  themeLight: '浅色',
  themeDark: '深色',
  themeSystem: '跟随系统',
  themeTime: '按时间',
  readingSuffix: '分钟阅读',
  wordsSuffix: '词',
  bylinePrefix: '文',
  updatedPrefix: '更新于',
  authorAbout: '关于作者',
  bookMode: '书籍模式',
  bookModeClose: '关闭',
  bookModePrev: '上一页',
  bookModeNext: '下一页',
  bookModeSmaller: '缩小文字',
  bookModeLarger: '放大文字',
  search: '搜索',
  searchHint: '输入关键词搜索文章。',
  searchEmpty: '未找到匹配的文章。',
  searchResults: '“{q}” 的 {n} 条结果',
  pagerNewer: '较新',
  pagerOlder: '较旧',
  pagerLabel: '分页导航',
  pagerPage: '第 {n} 页',
  metaTerm: '{site} 上归入{name}的全部文章。',
  metaSeries: '{name}：{site} 的一个系列，按适合阅读的顺序排列。',
  tocIndex: '目录',
  categoriesTitle: '分类',
  mostViewedTitle: '最多阅读',
  featuredTitle: '精选',
  tagsTitle: '标签',
  relatedTitle: '相关文章',
  readNext: '接着读',
  readNextSeries: '系列下一篇',
  resumePrompt: '从上次读到的地方继续？',
  seriesLabel: '系列',
  seriesPartPrefix: '第',
  seriesTitle: '系列',
  emptySeries: '该系列还没有文章。',
  // /archive: the year index, its jump row and its empty state. `archiveTitle` is also
  // the sidebar block's heading, so the page and the way in carry one name.
  archiveTitle: '归档',
  archiveMeta: '{site} 的全部文章，按年份排列。',
  archiveYears: '年份',
  archiveEmpty: '归档中还没有内容。',
  // /notes: the owner's notebook (ADR 0044) — its heading, its description, its empty state,
  // and the word before a clip's source.
  notesTitle: '笔记',
  notesMeta: '{site} 上保存的笔记与摘录。',
  notesEmpty: '还没有笔记。',
  noteSourcePrefix: '来自',
  // /notes/clip: the notebook's receiving door and its bookmarklet; the last three ride
  // the reader's pen on another site.
  clipHeading: '保存到笔记本',
  clipTitleLabel: '标题',
  clipNoteLabel: '你的批注',
  clipPrivate: '私密',
  clipPublic: '公开',
  clipSave: '保存',
  clipSavedHeading: '已保存。',
  clipSavedView: '打开笔记',
  clipAgain: '再保存一条',
  clipToolHeading: '从任何地方保存',
  clipToolHint: '把它拖到书签栏。在任何页面选中一段文字后点击它，这段文字就会连同来源一起到达这里。',
  clipToolLabel: '保存到 {site}',
  readerPenSend: '发送到我的笔记本',
  readerPenNotebookAsk: '你的 Quire Ink 在哪里？它的地址：',
  readerPenNotebookGo: '发送',
  previewNotice: '预览：此页面未公开，也不会被索引。',
  nlInvalid: '这似乎不是一个邮箱地址。',
  nlNoMail: '已订阅。邮件服务未配置，因此没有发送确认信。',
  copyCode: '复制',
  copiedCode: '已复制',
  nlHeading: '订阅邮件通讯',
  nlPlaceholder: 'your@email.com',
  nlButton: '订阅',
  nlSuccess: '就快好了 — 请查收邮件以确认。',
  nlError: '出错了，请重试。',
  nlConfirmSubject: '确认订阅',
  nlConfirmIntro: '确认订阅 {site}。',
  nlConfirmButton: '确认订阅',
  nlConfirmIgnore: '如果这不是你本人操作，可忽略此邮件。',
  nlThanksTitle: '订阅成功！',
  nlThanksBody: '感谢确认 — 有新内容时会通知你。',
  nlLinkInvalid: '此链接无效或已过期。',
  nlUnsubTitle: '已退订',
  nlUnsubBody: '你将不再收到邮件。随时可以重新订阅。',
  nlUnsubFooter: '退订',
  nlFooterWhy: '你收到这封邮件，是因为你订阅了 {site}。',
  nlUnsubConfirm: '确认你不想再收到邮件。',
  nlUnsubConfirmBtn: '确认取消订阅',
  mailTestSubject: 'SMTP 测试',
  mailTestBody: '这是来自你博客的测试邮件。如果你能读到，说明 SMTP 正常。',
  mailTestSamplePost: '示例文章',
  bcastRead: '阅读文章',
  bcastDigestSubject: '{n} 篇新文章',
  replySubject: '你的评论有新回复',
  replyIntro: '{name} 回复了你在《{title}》的评论。',
  replyRead: '查看回复',
  notFoundTitle: '页面未找到',
  notFoundText: '你要找的页面不存在或已被移动。',
  errorTitle: '出错了',
  errorText: '发生了意外错误，请重试。',
  backHome: '返回首页',
  backToTop: '回到顶部',
  quoteCopy: '复制引用',
  quoteCopied: '已复制',
  readerPenHighlight: '高亮',
  readerPenUnderline: '下划线',
  readerPenRing: '圈出',
  readerPenNote: '批注',
  readerPenDelete: '删除标记',
  readerPenNoteHint: '你的批注，保存在此浏览器中',
  readerPenKeptHere: '仅保存在此浏览器中。',
  readerPenKeep: '在所有设备上保留',
  readerPenKeepGoogle: '使用 Google 登录',
  readerPenKeepCode: '获取一个代码',
  readerPenKeepHave: '已有代码？粘贴到这里：',
  readerPenKeepUse: '使用',
  readerPenKept: '已在所有设备上保留。',
  readerPenKeepHint: '请记下此代码。它是打开你的标记的唯一钥匙，可在任何设备上使用。',
  readerPenForgetHere: '在此设备上忘记',
  readerPenForgetAll: '在所有地方忘记',
  readerPenKeepBad: '此代码在这里不存在。',
  readerPenShowCode: '显示代码',
  lightboxPrev: '上一张',
  lightboxNext: '下一张',
  lightboxClose: '关闭',
  commentsHeading: '评论',
  commentsEmpty: '暂无评论。来做第一个评论的人吧。',
  commentName: '姓名',
  commentEmail: '邮箱',
  commentEmailNote: '不会公开',
  commentWebsite: '网站（选填）',
  commentBody: '评论内容',
  commentSubmit: '发表评论',
  commentReply: '回复',
  commentDeleted: '[评论已删除]',
  commentError: '评论发送失败，请重试。',
  commentChecking: '正在检查你的浏览器，请稍候。',
  commentSignInGoogle: '用 Google 登录',
  commentAs: '评论身份',
  commentSignOut: '退出登录',
  commentSignInError: '登录失败。请重试，或使用姓名和邮箱发表评论。',
} satisfies Dict

export default zh
