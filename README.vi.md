...
**2.2.13** mở đường cho chữ đi ra: blog theo dõi được từ Mastodon, đọc được bằng chương trình, tải về được dưới dạng Markdown mà công cụ khác mở lên đọc thẳng, và mỗi bài nay tự khai được nó viết bằng tiếng gì. Nó đang chạy trang demo ở trên lẫn blog của chính tác giả tại [manhhung.me](https://manhhung.me). [Nhật ký thay đổi](./CHANGELOG.md) ghi đủ, kèm số đo từng mục.

- **Theo dõi được blog từ bất cứ đâu nói được ActivityPub.** Người đọc trên Mastodon bấm theo dõi ngay chỗ họ vẫn đọc, và mỗi bài đăng, sửa hay gỡ đều tới được họ. **Tắt sẵn lúc cài và tắt sẵn khi nâng cấp** (Cài đặt, Máy chủ và kết nối): bật lên là trao cho blog một danh tính trong mạng máy chủ của người khác, kèm một cặp khoá và một danh sách người lạ đã xin nghe, và cái tên định danh sau đó không đổi được nữa. Bản này chỉ phát đi chứ chưa đọc về, nên trả lời, thích và chia sẻ đều bị bỏ.
- **Đọc được blog bằng chương trình.** `/api/v1` trả về bài, trang, ghi chú và các nhãn dưới dạng JSON, kèm đúng đoạn Markdown mà mỗi bài được viết ra, nên dựng một giao diện thứ hai hay một chỉ mục tìm kiếm không còn phải cào từng trang HTML. **Tắt sẵn lúc cài và khi nâng cấp**, lúc tắt trả 404 chứ không phải 403, chỉ đọc, không có khoá riêng, và trả cho chủ blog đúng từng byte như trả cho người lạ.
- **Một bài tự khai nó viết bằng tiếng gì**, và trỏ sang các bản dịch của nó: đúng `lang`, đủ bộ hreflang, đủ bản thay thế trong sitemap, và một tiêu đề tiếng Hàn nằm trong danh sách của blog tiếng Việt được vẽ bằng đúng mặt chữ, đọc lên bằng đúng giọng. Bài không khai gì thì giữ nguyên như cũ, vì không khai nghĩa là "chưa ai nói", không phải là tiếng Anh.
- **Chữ đi ra bằng Markdown, và quay về cũng vậy.** Một tệp ZIP nằm cạnh bản sao lưu: từng bài, từng trang, từng ghi chú kèm phần đầu bài, toàn bộ tệp tải lên, toàn bộ cài đặt, và một README cho người mở nó ra sau mười năm. Có sẵn bộ đọc lại chính tệp đó, nên "không mất gì qua một vòng đi về" là một bài test chứ không phải một lời hứa.
- **Đoạn văn chỉ có mỗi một liên kết trở thành một thẻ**, có tiêu đề, có mô tả, ảnh được tải về máy chứ không nhúng thẳng từ bên kia, và liên kết tới một PDF đã tải lên trông ra dáng một bản tải về. **Bật sẵn cho blog mới, tắt sẵn cho blog đã có** (Cài đặt, Bài viết), vì thẻ liên kết là thứ duy nhất phải với tay ra ngoài.
- **Nhiều bài cùng lúc**: đăng, chuyển thành nháp hay bỏ vào thùng rác cho cả một nhóm trong một lần gọi, chỗ mà năm mươi dấu tích trước đây là năm mươi lần gọi, năm mươi lần xoá cache và năm mươi lần dọn CDN. Sổ ghi chú cũng đăng ký theo dõi được, và cả hai nguồn tin nay có cả RSS lẫn JSON Feed.
- **Ảnh Docker nói đúng nó là gì.** Toàn bộ nhãn OCI trước nay là của ảnh nền, nên một container đã xuất bản tự xưng là `bun`, phiên bản `1.4.2-slim`, nguồn `github.com/oven-sh/bun`. Đó chính là trường mà kho ảnh đọc để nối gói với kho mã, và là trường mà công cụ báo nâng cấp đem ra so. Ảnh cũng nhẹ đi 4,1 MB: 291 tệp test và hai nửa chạy trong trình duyệt vẫn đi kèm dù bản đã đóng gói của chúng nằm ngay bên cạnh.
- **Ba chỗ đang làm mất người đọc.** Một mã ghi chú cuối bài có thể gắn được trình xử lý sự kiện chạy thật lên trang của người đọc; lớp xác thực thứ hai có thể dò khoảng tám lần mỗi giây, đo được 645 lần đoán trong 8 giây mà không lần nào bị chặn; và chỉ một bài có ký tự điều khiển lạc trong tiêu đề là cả nguồn RSS tắt tiếng với toàn bộ người đăng ký, trong khi trang của chính bài đó trông vẫn hoàn hảo.
