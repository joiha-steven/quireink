> The agent-discovery surface: what Quire Ink exposes so AI agents can find, read and drive
> the site (the standards behind Cloudflare's "Is Your Site Agent-Ready?" scan). Read when
> touching `/.well-known/*`, `llms.txt` or the MCP door. MCP internals → [`mcp.md`](./mcp.md); worked prompts → [`agent-cookbook.md`](./agent-cookbook.md); SEO/feeds → [`seo-pwa.md`](./seo-pwa.md).
