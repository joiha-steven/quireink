...
| [0054](0054-the-admin-is-pages-again.md) | The admin is pages again, and the editor is the only client left | 2026-09-14 | ✅ replaces [0006](0006-admin-stays-react-spa.md), whose own *"cost accepted"* clause said this bill would come due. React goes, the admin becomes server-rendered HTML with islands like the reading site, and `prosemirror-*` becomes a named tier-B dependency in place of the `@tiptap/*` wrapper. ⚠️ **its five steps are screens, and they assume a step 0 it does not name**: the chrome — shell, sidebar, router — is shared by all fourteen screens, so nothing converts before it does. Found on contact with the code the same day |
