 ...
 *
 * Nothing else in this seeder produces a single entry, and the reason is worth knowing:
...
export async function seedActivity(origin: number, now: number): Promise<number> {
  ...
  // Newest last: the list renders newest first, and writing it in the order things happened
  // is the only way to keep the story straight while editing it.
  const ENTRIES: [ActivityAction, string, number][] = [
  ...
  for (const [action, detail, at] of ENTRIES) {
