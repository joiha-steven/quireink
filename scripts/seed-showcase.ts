...
// `seed-comments.ts`, the owner-facing half in `seed-admin.ts`, and the library plates in
// `seed-media.ts`. It got split when filling the admin's empty screens took it past the
// 400-line cap, and the split is by AUDIENCE rather than by size: what a reader sees, what
// the owner sees, and the code that assembles both.
//
...
import { seedAdmin, seedActivity } from './seed-admin'
