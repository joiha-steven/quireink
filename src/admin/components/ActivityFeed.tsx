...
export function ActivityFeed({ entries, limit = 8 }: { entries: ActivityEntry[]; limit?: number }) {
  ...
  const now = Date.now()
  return (
    // TWO COLUMNS on a wide screen, and it is the emptiness that forces it rather than a wish
    // for density: six rows of ~300px across a 1400px band is a card three-quarters air.
    <ul className="grid xl:grid-cols-2 xl:gap-x-10">
      {entries.slice(0, limit).map((entry) => (
        <li
          key={entry.id}
          className="flex items-start gap-3 border-b border-neutral-100 py-2.5 last:border-b-0 dark:border-neutral-800 xl:[&:nth-last-child(-n+2)]:border-b-0"
        ...
        >
          ...
          <div className="min-w-0 flex-1">
            {/* The detail is the sentence. An action with none — `cache.clear` says
                everything in its own name — takes the top line itself rather than
                leaving it blank above its own filing. */}
            <p
            ...
            {entry.detail && (
              <p className="truncate text-xs text-neutral-500 dark:text-neutral-400">{entry.action}</p>
            )}
          </div>
