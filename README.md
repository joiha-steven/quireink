...
One fix, and it is the whole release: **2.2.8 signed in to a blank admin.** The dashboard threw `Minified React error #321` and nothing rendered. The entry script is written to disk under one name and was served under another — a fingerprint the server computed so the file would not be re-downloaded on every load — which was safe only as long as no code-split chunk imported the entry back. The bundler's next version imports it back from every lazy route, so the browser held the same file under two URLs, and a JavaScript module is identified by the URL it was fetched from: two module records, two copies of React, and the first screen to call a hook got the wrong one. The entry now carries the bundler's own hash and nothing renames it. A build check reads the built directory for exactly one entry, and a test walks every URL the admin shell can reach and fails if two of them serve the same bytes.

**And what it does not do.** This release changes nothing else: everything 2.2.8 shipped, it still ships, and everything 2.2.8 could not do it still cannot. Nothing on a reader's side was ever affected — public pages are finished HTML and were never blank — so a blog whose owner did not sign in during 2.2.8 lost nothing and gains nothing here. There is no workaround to apply if you are stuck: the two names are baked into the bundle inside the image, so the fix arrives only by taking this release. `docker compose up` will not fetch it on its own — upgrading is `pull` then `up -d`. And every standing limit holds: a NAS and a Kubernetes cluster get no Caddy, the Motion switch is the owner's rather than per-reader, an install that rewrites its own HTML with nginx `sub_filter` loses the origin's compression and validator, a Cloudflare zone with "JavaScript Detections" on injects a script this software did not put there, and an origin with no CDN still owes a distant reader the round trip that no saved byte buys back.
