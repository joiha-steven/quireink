...
The pen, all the way through. **Readers get a pen**: select words on a post and a bar offers the five inks, the underline, the ring, a note and the quote, drawn with the site's own hand; marks are anchored to their words rather than to positions, kept in the reader's browser, and carried across devices by a twenty-character notebook code or the Google sign-in commenters already have, with the server holding hashes and the owner seeing nothing. **A notebook**: notes and clips as a third kind of writing under `/notes`, kept apart from the posts, with a door at `/notes/clip` that a reader's pen or a bookmarklet on any page can send a passage through. **The open standards**: IndieAuth on the MCP server's OAuth door, Micropub in, Webmention both ways, `h-entry` on every note. **`/pen.css`**: any site may link it and write with your inks. In the editor a mark just applied draws itself in 200 milliseconds and a felt tip squeaks, both on the switches you already have. The Write list asks two questions on two rows instead of six segments on one, the sheet beside it takes its width again, and it follows a save. On a tablet the contents stand under the title instead of vanishing; a phone shows no grid button; a newspaper row ends on a full line.

**And what it does not do.** It is a beta: expect 2.2.10 to still move things. Two devices marking the same page at once overwrite each other, last save wins. Nothing in the admin shows which passages readers keep most; only the `list_mentions` MCP tool answers that. Webmention verifies its source and rate-limits, but no spam judgement is wired to it. The squeak was measured, not listened to by anyone but its author, and the Google round trip of *Keep these* was measured to the redirect and not clicked through by hand. The Help screens are still English only, a few counts still read "1 words", and every standing limit holds — a NAS and a Kubernetes cluster get no Caddy, the Motion switch is the owner's rather than per-reader, an install that rewrites its own HTML with nginx `sub_filter` loses the origin's compression and validator, and an origin with no CDN makes a reader on the far side of the planet pay a round trip that saved bytes cannot buy back.
...
| 🖍️&nbsp;**The&nbsp;pen** | `==text==` highlights in five inks, `++text++` underlines in pencil, `@@word@@` rings a word in red ballpoint. Strokes grown from a seeded hand that inks unevenly, so no two on a page share a shape. Pigments measured off a photograph of a real pen box. Readers get the same pen if you let them: marks kept in their browser, carried across devices by a code rather than an account, and sent to a notebook of their own. Any site may link `/pen.css` and write with your inks |
| 📓&nbsp;**Notebook** | A third kind of writing beside posts and pages: notes and clips, with where a passage came from kept as a field. Speaks IndieAuth, Micropub and Webmention, so a note can arrive from any client and the source hears about it. [How it works](./docs/features/notes.md) |
...
| `PURGE_WEBHOOK_URL` | ◻️ | A URL the blog POSTs to whenever it flushes its own cache, for a CDN that is not Cloudflare ([ADR 0033](./docs/decisions/0033-purging-an-edge-that-is-not-cloudflare.md)). Normally entered in Settings → Server & connections instead |
| `S3_BUCKET`, `S3_ACCESS_KEY_ID`, `S3_SECRET_ACCESS_KEY` (+`S3_ENDPOINT`, `S3_REGION`, `S3_PREFIX`) | ◻️ | An S3-compatible bucket every snapshot is also shipped to ([ADR 0035](./docs/decisions/0035-the-snapshot-leaves-the-machine.md)). Normally entered in Settings → Server & connections instead |
...
SMTP, Turnstile and CDN credentials go in **Settings → Comments & mail** (mail) and **Server & connections** (the rest), and stay on the server. Your posts live in `DATA_DIR` and your uploads folder, never in git.
