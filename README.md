...
**2.2.10-beta.4** is the fourth pre-release before 2.2.10. It runs the demo above and the author's own blog at [manhhung.me](https://manhhung.me); the [changelog](./CHANGELOG.md) has everything that changed. Its Docker tag is `2.2.10-beta.4` and only that: `latest` is still 2.2.9, so nobody gets a beta by accident.

**What it adds is a Markdown engine of its own.** Four libraries used to answer the same question here: one rendered the reader's page, one opened a post in the editor, one saved it back, and a run of regular expressions wrote the excerpt. Four answers to one question drift, and these did, in public. One engine does all five jobs now, from one parse of your text, and it is measured against the specifications rather than against itself: CommonMark 0.31.2 at 648 of 652 examples and GFM at 24 of 24, on every run. Before the switch was thrown the two engines were compared on 92 real posts, three different ways, which is how a hole nobody had noticed turned up: `[js](javascript:alert(1))` used to publish as a working link. Saving a long piece went from 144ms to 7ms, a hostile paste can no longer stall the process, and the engine carries no dependencies at all. The writing surface also learned find and replace on its own chord, and it now draws the pen with the same forty variants the published page uses.

**And what it does not do.** It is a beta, and this one replaced the library that renders every word on the site: 92 posts were compared three ways before the switch, but your Markdown is not those 92 posts, so keep the backup you take before upgrading. The engine keeps three deliberate disagreements with CommonMark, each named in its suite. There is no multi-user mode: one blog, one owner, one process, and comments have accounts where the writing side does not. There are four looks and no fifth, a look is site-wide and dresses the published site only, and going further is still custom CSS. Two devices marking the same page at once overwrite each other, last save wins. Nothing in the admin shows which passages readers keep most; only the `list_mentions` MCP tool answers that. Webmention verifies its source and rate-limits, but no spam judgement is wired to it. The Help screens are still English only, a few counts still read "1 words", and every standing limit holds: a NAS and a Kubernetes cluster get no Caddy, the Motion switch is the owner's rather than per-reader, an install that rewrites its own HTML with nginx `sub_filter` loses the origin's compression and validator, and an origin with no CDN makes a reader on the far side of the planet pay a round trip that saved bytes cannot buy back.
