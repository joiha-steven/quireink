...
**2.2.9** is the current release. It runs the demo above and the author's own blog at [manhhung.me](https://manhhung.me); the [changelog](./CHANGELOG.md) has everything that changed.

Two audits in a day and a half, and everything they turned up. **Settings are regrouped into seven tabs by the question you are holding** rather than by the part of the code a key belongs to, each tab printing its question under its name and carrying a Save key that says how much is waiting on it. The browser's twenty `confirm()` and `prompt()` boxes are gone, replaced by dialogs that name what they are about and cannot freeze the page behind them. **Every layout shift measured as a page arrives is now zero** — 0.136 on a phone's front page before this, 0.059 on a desktop one — because a listing carries thirty posts and a link rather than the whole archive with most of it hidden. **The admin's first frame is 375 KB where it was 1,063**: it had been fetching all eleven reader dictionaries and the editor before drawing anything. And a dozen ways of losing work are closed: two tabs saving settings at once and erasing each other, a rename that half-finished, an editor that reopened the piece it had just saved when you asked for a new one, a newsletter send that died with the HTTP request that started it. It also carries the fix 2.2.8 needed, which signed in to a blank admin.

**And what it does not do.** The regrouping moves where a setting lives, so anybody who knew the old eight tabs has to look once more; the search box beside the tabs and `?setting=<key>` are what make that once rather than always. Two save models still share the last three tabs: the sheet's Save stores the ordinary settings, while a card that reaches an SMTP host or a Cloudflare zone keeps its own key, because storing a credential and testing it are not one act. The Help screens are still English only while the rest of the admin speaks eleven languages, and a few counts still read "1 words" because the plural forms are not there yet. The contents rail beside an article is still gone below 1024px, the grid switch still changes little on a phone, and a newspaper front page at 820px can still leave a third card on a row of its own. Nothing here is a reader-visible feature: a blog nobody signed in to during 2.2.8 gains speed and loses nothing. And every standing limit holds — a NAS and a Kubernetes cluster get no Caddy, the Motion switch is the owner's rather than per-reader, an install that rewrites its own HTML with nginx `sub_filter` loses the origin's compression and validator, a Cloudflare zone with "JavaScript Detections" on injects a script this software did not put there, and an origin with no CDN still owes a distant reader the round trip that no saved byte buys back. Upgrading is `pull` then `up -d`; `docker compose up` will not fetch it on its own.
