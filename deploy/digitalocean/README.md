...
2. Under **Advanced Options → Add Initialization scripts**, paste the whole of
   `user-data.sh`. The box is free — it is [cloud-init user
...
4. The one-time link that claims it is in `/root/quire-claim.txt` on the droplet — and in
...
**What has been proven, and what has not.** The payload — pull the published image, boot it
on root-owned bind mounts, answer `/api/health`, print the claim link, keep its data across
a restart, run as UID 1000 — is verified against `quireink/quireink:latest`. The address
